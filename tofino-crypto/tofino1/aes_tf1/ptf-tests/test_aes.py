...
class BridgeHeader(Packet):
    ...
    fields_desc = [
        ...
        ByteField("Round", default=0),
        ByteField("Length", default=0),
        ByteField("UserData", default=0),
        ConditionalField(
...
class AesTf1Test(BaseTest):

    class Pipe0(BfRuntimeTest):
        def setUp(self, client_id, p4_name="aes_tf1"):
    ...
    class Pipe1(BfRuntimeTest):
        def setUp(self, client_id, p4_name="aes_tf1_test"):
            BfRuntimeTest.setUp(self, client_id, p4_name)
            self.bfrt_info = self.interface.bfrt_info_get(p4_name)
    ...
    def runTest(self):
        ...
        exp = ether / BridgeHeader(
            Flags=BrFlags.ENB_BLOCK0 | BrFlags.ENB_BLOCK1 | BrFlags.LAST_ROUND,
            Round=11,
            Block0=0x3925841d02dc09fbdc118597196a0b32,
