    ...
    apply {
        if (hdr.key.isValid()) {
            if (hdr.meta.round == 0) {
                ...
                hdr.meta.round = hdr.meta.round + 1;
            }

            // Calculate key for round 1, 3, 5, 7, 9
            ...
            hdr.meta.round = hdr.meta.round + 1;
            if (hdr.meta.round == 10) hdr.meta.last_round = 1;
        }
    }
}
    ...
    inout ingress_intrinsic_metadata_for_tm_t       ig_tm_md)
{
    ...
    ST_TABLES(0, 2, hdr.block0)
    ST_TABLES(1, 2, hdr.block1)

    // === Main ===

    apply {
        if (hdr.key.isValid()) {
            // Calculate key for round 2, 4, 6, 8, 10
            EXPAND_KEY(0, 2, hdr.key);
            EXPAND_KEY(1, 2, meta.key);

            // Round 2, 4, 6, 8, 10
            if (hdr.block0.isValid()) {
                APPLY_ST_TABLES(0, 2, hdr.block0);
                ADD_ROUND_KEY(0, hdr.key);
            }
            if (hdr.block1.isValid()) {
                APPLY_ST_TABLES(1, 2, hdr.block1);
                ADD_ROUND_KEY(1, meta.key);
            }

            hdr.meta.round = hdr.meta.round + 1;

            // Recirculate or forward to next pipe
            tab_forward.apply();
        }
    }
}
