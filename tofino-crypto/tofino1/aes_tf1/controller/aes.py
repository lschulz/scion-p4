...
def _add_fwd_entry_testbench(t_fwd, target, ig_port: int, eg_port: int):
    """Add an entry to the textbench forwarding table."""
...
def _add_fwd_entry(t_fwd, target, last_round: bool, ig_port: int, eg_port: int):
    """Add an entry to the forwarding table."""
    t_fwd.entry_add(target,
        [t_fwd.make_key(
            [gc.KeyTuple("hdr.meta.last_round", last_round),
             gc.KeyTuple("ig_intr_md.ingress_port", ig_port)])],
        [t_fwd.make_data(
            [gc.DataTuple("egress_port", eg_port)],
            "Ingress.set_egress")])
...
def _program_aes_table_512(tab, target, key: str, action: str, data: Iterable[int]):
    """Program a hash action table with 512 entries.
    ...
    for i, v in enumerate(data):
        tkeys[i] = tab.make_key([
            gc.KeyTuple("hdr.meta.last_round", i // 256),
...
class _AesController:
    ...
    """
    _key_exp = (1, 3)      # key expansion tables
    _key_exp_no_meta = []  # key expansion steps without key copy in metadata
    _t_tables = (1, 2)     # T-table copies
    ...
    def _get_tables(self):
        ...
        for block in range(2):
            for s in range(4):
                for column in range(4):
                    tab = self.bfrt_info.table_get(f"tab_b{block}_r{round}_st{s}_c{column}")
...
class _PipeForwardingController:
    ...
    def program_tables(self, target):
        ...
        # Recirculate until last_round is set
        for port in range(8, 72):
            _add_fwd_entry(self.t_fwd, target, False,
                _make_port(self.pipe, port), _make_port(self.pipe, port))
            _add_fwd_entry(self.t_fwd, target, True,
...
class AesTestPipe:
    ...
    def __init__(self,
        ...
        self.t_fwd = self.bfrt_info.table_get("tab_testbench_fwd")

    def program_tables(self):
        target = gc.Target(device_id=0)
        for port in range(8, 72):
            _add_fwd_entry_testbench(self.t_fwd, target,
                _make_port(self.pipe, port), _make_port(self.next_pipe, port))
